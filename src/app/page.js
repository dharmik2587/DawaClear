...
function Btn({ children, onClick, primary, sm, disabled, style = {} }) {
  return (
    <button onClick={onClick} disabled={disabled} style={{
      ...
      padding: primary ? (sm ? '9px 20px' : '11px 26px') : '7px 14px',
      borderRadius: 100, fontSize: primary ? (sm ? 12 : 13) : 11.5,
      cursor: disabled ? 'not-allowed' : 'pointer', fontFamily: "'Sora',sans-serif",
      border: primary ? 'none' : '1px solid rgba(255,255,255,0.09)',
      background: disabled ? 'rgba(255,255,255,0.05)' : primary ? 'linear-gradient(135deg,#00d4b8,#00a896)' : 'rgba(255,255,255,0.04)',
      color: primary ? '#050d1a' : '#9ab8b4', fontWeight: primary ? 700 : 400,
      opacity: disabled ? 0.5 : 1, transition: 'all .2s', ...style
    }}>{children}</button>
  );
}

function Card({ children, gold, glow, style = {} }) {
  return (
    <div style={{
      background: gold ? 'linear-gradient(145deg,rgba(232,201,122,0.14),rgba(232,201,122,0.06))' : glow ? 'linear-gradient(145deg,rgba(0,212,184,0.05),var(--gl))' : 'var(--gl)',
      border: `1px solid ${gold ? 'var(--b3)' : glow ? 'rgba(0,212,184,0.16)' : 'var(--b2)'}`,
      borderRadius: 18, padding: 22, marginBottom: 18, ...style
    }}>{children}</div>
  );
}

function HDR({ icon, title, chip, cc }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
      ...
      <div style={{ fontSize: 14.5, fontWeight: 600, flex: 1 }}>{title}</div>
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, pointerEvents: 'none', zIndex: 9999, overflow: 'hidden' }}>
      {Array.from({ length: 55 }).map((_, i) => (
        <div key={i} style={{ position: 'absolute', width: 7, height: 7, borderRadius: '50%', background: cols[i % 4], left: `${Math.random() * 100}vw`, top: -10, animation: `cf ${1 + Math.random() * 2}s ease-in forwards`, animationDelay: `${Math.random() * 0.5}s` }} />
      ))}
    </div>
  );
}
...
function Toast({ msg }) {
  if (!msg) return null;
  return (
    <div style={{ position: 'fixed', bottom: 90, right: 24, zIndex: 9998, background: 'rgba(10,22,40,0.97)', border: '1px solid rgba(0,212,184,0.22)', color: '#e8f4f2', padding: '11px 18px', borderRadius: 12, fontSize: 13, fontFamily: "'Sora',sans-serif", backdropFilter: 'blur(16px)', boxShadow: '0 8px 28px rgba(0,0,0,0.4)', maxWidth: 340, animation: 'fu .3s ease' }}>
      {msg}
    </div>
  );
}
...
function Landing({ onStart, onDemo }) {
  ...
  const pickFile = f => {
    if (!f) return;
    setFerr('');
    if (f.size > 10 * 1024 * 1024) { setFerr('Max file size is 10MB.'); return; }
    setFile(f);
    ...
    <div style={{ position: 'relative', zIndex: 1 }}>
      ...
      <div style={{ minHeight: '100vh', padding: '76px 2.5rem 0', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 0, maxWidth: 1240, margin: '0 auto', alignItems: 'center' }}>
        <div style={{ paddingRight: 52, paddingBottom: 52 }}>
          ...
          <p style={{ fontSize: 15, lineHeight: 1.75, color: 'var(--tx2)', maxWidth: 430, marginBottom: 36 }}>
            Upload your prescription or discharge summary. <strong style={{ color: 'var(--tx)', fontWeight: 500 }}>Claude Vision</strong> reads the actual image — handwritten or printed — and builds your full health dashboard in 30 seconds.
          </p>
          ...
          <div style={{ display: 'flex', gap: 28, marginTop: 44, paddingTop: 28, borderTop: '1px solid var(--b2)' }}>
            {[['2M+', 'Docs processed'], ['98%', 'Accuracy'], ['40+', 'Languages']].map(([v, l]) => (
              <div key={l}>
            ...
            <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(160deg,rgba(15,32,64,0.92),rgba(10,22,40,0.96))', border: '1px solid var(--b)', borderRadius: 20, padding: 20, boxShadow: '0 40px 80px rgba(0,0,0,0.6)', overflow: 'hidden' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                <div style={{ fontSize: 9.5, color: 'var(--tx3)', fontFamily: "'JetBrains Mono',monospace" }}>
                  <strong style={{ display: 'block', fontSize: 11, color: 'var(--tx2)', marginBottom: 1 }}>Dr. Priya Mehta, MD</strong>Cardiology · Apollo
                </div>
              ...
              <div style={{ background: 'var(--gl)', border: '1px solid var(--b2)', borderRadius: 8, padding: '7px 10px', marginBottom: 10, display: 'flex', gap: 9, alignItems: 'center' }}>
                ...
                <div><div style={{ fontSize: 11.5, fontWeight: 600 }}>Rajesh Kumar</div><div style={{ fontSize: 9.5, color: 'var(--tx3)', fontFamily: "'JetBrains Mono',monospace", marginTop: 1 }}>M · 54y · BP: 145/95</div></div>
              </div>
              ...
              {[['Amlodipine', '5mg', '07:00'], ['Metoprolol', '25mg', '8·8PM'], ['Aspirin', '75mg', 'Lunch']].map(([n, d, t]) => (
                <div key={n} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '5px 0', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                  <div><div style={{ fontSize: 10.5, fontWeight: 600 }}>{n}</div><div style={{ fontSize: 9, color: 'var(--tx3)', fontFamily: "'JetBrains Mono',monospace", marginTop: 1 }}>{d}</div></div>
      ...
      <div style={{ maxWidth: 1240, margin: '0 auto', padding: '80px 2.5rem' }}>
        ...
        <p style={{ fontSize: 14.5, color: 'var(--tx2)', maxWidth: 480, lineHeight: 1.75, marginBottom: 48 }}>
          Claude Vision reads your actual image — handwritten prescriptions, printed slips, discharge summaries. Real extraction, not templates.
        </p>
        ...
        >
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr' }}>
            ...
            <div onClick={() => ref.current.click()} style={{ padding: 44, borderRight: '1px solid var(--b2)', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', cursor: 'pointer' }}>
              ...
              <div style={{ fontSize: 18, fontWeight: 700, marginBottom: 8, wordBreak: 'break-all', maxWidth: 280 }}>{file ? file.name : 'Drop your document here'}</div>
              {file
              ...
              <Btn primary onClick={e => { e.stopPropagation(); ref.current.click(); }} style={{ fontSize: 13, padding: '10px 22px' }}>{file ? '📂 Change File' : '📤 Choose File'}</Btn>
            ...
            <div style={{ padding: '32px 40px', display: 'flex', flexDirection: 'column', gap: 18 }}>
              <div><label style={lbS}>Patient Name (optional)</label><input style={inS} placeholder="e.g. Priya Sharma" value={name} onChange={e => setName(e.target.value)} /></div>
              <div><label style={lbS}>Patient Age (hint)</label><input style={inS} type="number" placeholder="e.g. 45" value={age} onChange={e => setAge(e.target.value)} /></div>
              <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 6 }}>
                <Btn primary onClick={() => { if (!file || !fdu) { alert('Please select a document first.'); return; } onStart({ file, fdu, name, age, lang, mode, role }); }} style={{ padding: '12px 28px', fontSize: 14 }}>
                  🔍 Analyse My Document →
                </Btn>
      ...
      <div style={{ maxWidth: 1240, margin: '0 auto', padding: '0 2.5rem 80px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14 }}>
          {[
            { w: true, i: '🧠', t: 'Real Claude Vision Analysis', d: 'Not OCR templates — Claude reads and understands your document image. Works on blurry photos, handwritten notes, and multilingual prescriptions.' },
            ...
            { i: '🌐', t: 'Hindi · English · Gujarati', d: 'Toggle re-renders your full brief instantly in chosen language.' },
            { i: '🔊', t: '"Sunao Mujhe" Voice', d: 'One tap reads your brief aloud in your chosen language.' },
            { i: '🆘', t: '2AM Panic Mode', d: 'Red button — immediate emergency info and call buttons for 102/112.' },
          ].map(({ w, i, t, d }) => (
...
function Processing({ step, pct, status }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 500, background: 'rgba(5,13,26,0.97)', backdropFilter: 'blur(20px)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 18, padding: 20 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8, width: 340 }}>
        {steps.map(s => (
          <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 15px', borderRadius: 11, background: step === s.id ? 'var(--tl3)' : step > s.id ? 'rgba(0,212,184,0.04)' : 'var(--gl)', border: `1px solid ${step === s.id ? 'var(--b)' : 'var(--b2)'}`, fontSize: 13, color: step === s.id ? 'var(--tl)' : step > s.id ? 'var(--tx3)' : 'var(--tx2)', transition: 'all .4s' }}>
...
function Panic({ result, onClose }) {
  const r = result || {};
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(5,13,26,0.97)', backdropFilter: 'blur(20px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div style={{ maxWidth: 440, width: '100%', background: 'rgba(255,71,87,0.08)', border: '1px solid rgba(255,71,87,0.3)', borderRadius: 20, padding: 32, textAlign: 'center' }}>
        ...
        <div style={{ background: 'rgba(255,71,87,0.07)', border: '1px solid rgba(255,71,87,0.18)', borderRadius: 10, padding: '14px 16px', marginBottom: 18, textAlign: 'left' }}>
          ...
          <div style={{ fontSize: 13, color: 'rgba(255,144,144,0.9)', lineHeight: 1.6, marginTop: 8, paddingTop: 8, borderTop: '1px solid rgba(255,71,87,0.2)' }}>{r.emergencyInfo || 'Call 102 immediately.'}</div>
        </div>
  ...
  const renderTab = () => {
    switch (tab) {
      case 'overview': return <>
        <div style={{ background: 'linear-gradient(145deg,var(--n2),var(--n3))', border: '1px solid var(--b)', borderRadius: 22, padding: 26, marginBottom: 18, position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 18, gap: 10 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 10, color: 'var(--tl)', background: 'var(--tl3)', border: '1px solid var(--b)', padding: '4px 11px', borderRadius: 100, fontFamily: "'JetBrains Mono',monospace", whiteSpace: 'nowrap' }}>
              <span style={{ width: 5, height: 5, borderRadius: '50%', background: 'var(--tl)', animation: 'tp 2s infinite', flexShrink: 0 }} />DawaClear Verified
            </div>
          </div>
        ...
        <Card glow><HDR icon="🩺" title="Plain-Language Diagnosis" chip="AI Simplified" /><div style={{ fontSize: 13.5, color: 'var(--tx2)', lineHeight: 1.85, borderLeft: '3px solid var(--tl)', paddingLeft: 15, fontStyle: 'italic', marginBottom: 13 }}>{r.diagnosisPlain || 'No diagnosis found.'}</div><div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>{(r.diagnosisTags || []).map((t, i) => <span key={t} style={{ fontSize: 10.5, padding: '3px 10px', borderRadius: 100, border: `1px solid ${i === 0 ? 'rgba(78,205,196,0.25)' : 'var(--b2)'}`, background: i === 0 ? 'rgba(78,205,196,0.1)' : 'var(--gl2)', color: i === 0 ? 'var(--gr)' : 'var(--tx3)' }}>{t}</span>)}</div></Card>
        <Card><HDR icon="💊" title="Medication Summary" chip={`${(r.medications || []).length} medicines`} />{(r.medications || []).length === 0 ? <div style={{ fontSize: 13, color: 'var(--tx3)' }}>No medications found.</div> : <table style={{ width: '100%', borderCollapse: 'collapse' }}><thead><tr>{['Medicine', 'Dose', 'Frequency', 'Timing', 'Days'].map(h => <th key={h} style={{ fontSize: 9.5, color: 'var(--tx3)', fontWeight: 600, textAlign: 'left', padding: '8px 9px', borderBottom: '1px solid var(--b2)', letterSpacing: '0.08em', textTransform: 'uppercase' }}>{h}</th>)}</tr></thead><tbody>{(r.medications || []).map(m => <tr key={m.name}><td style={{ padding: '10px 9px', fontSize: 13, color: 'var(--tx)', fontWeight: 600, borderBottom: '1px solid rgba(255,255,255,0.03)' }}>{m.name}</td><td style={{ padding: '10px 9px', fontSize: 12.5, color: 'var(--tx2)', borderBottom: '1px solid rgba(255,255,255,0.03)' }}>{m.dose}</td><td style={{ padding: '10px 9px', fontSize: 12.5, color: 'var(--tx2)', borderBottom: '1px solid rgba(255,255,255,0.03)' }}>{m.frequency}</td><td style={{ padding: '10px 9px', borderBottom: '1px solid rgba(255,255,255,0.03)' }}><span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 10, background: 'var(--tl3)', color: 'var(--tl)', padding: '2px 7px', borderRadius: 5 }}>{m.timing}</span></td><td style={{ padding: '10px 9px', fontSize: 12.5, color: 'var(--tx2)', borderBottom: '1px solid rgba(255,255,255,0.03)' }}>{m.days}</td></tr>)}</tbody></table>}</Card>
        <Card gold><HDR icon="💬" title="Share with Family" chip="One-liner" cc="gold" /><div style={{ fontSize: 13.5, color: 'var(--tx2)', lineHeight: 1.8, fontStyle: 'italic', marginBottom: 14 }}><span style={{ color: 'var(--go)', fontStyle: 'normal', fontWeight: 600 }}>"</span>{r.familyOneliner || 'No summary.'}<span style={{ color: 'var(--go)', fontStyle: 'normal', fontWeight: 600 }}>"</span></div><div style={{ display: 'flex', gap: 7, flexWrap: 'wrap' }}>{[['📋 Copy', () => copy(r.familyOneliner || '')], ['💬 WhatsApp', () => window.open(`https://wa.me/?text=${encodeURIComponent(r.familyOneliner || '')}`, '_blank')], ['🔊 Read Aloud', () => speak(r.familyOneliner || '')], ['🧲 Fridge PDF', () => pFridge(r)]].map(([l, fn]) => <button key={l} onClick={fn} style={{ display: 'flex', alignItems: 'center', gap: 4, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.09)', color: 'var(--tx2)', padding: '6px 13px', borderRadius: 100, fontSize: 11.5, cursor: 'pointer', fontFamily: "'Sora',sans-serif" }}>{l}</button>)}</div></Card>
      </>;
        ...
        return <>
          <Card>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12 }}>
              {slots.map(s => { const meds = r[s.k] || []; const allDone = meds.length > 0 && meds.every((_, i) => tlck[`${s.k}-${i}`]); return <div key={s.k} style={{ background: allDone ? 'rgba(0,212,184,0.03)' : 'var(--n2)', border: `1px solid ${allDone ? 'rgba(0,212,184,0.2)' : 'var(--b2)'}`, borderRadius: 12, padding: 16, transition: 'all .3s' }}><div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}><div style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 11.5, color: 'var(--tl)', fontWeight: 600 }}>{s.l}</div><div style={{ fontSize: 10, color: 'var(--tx3)' }}>{s.sub}</div></div>{meds.length === 0 ? <div style={{ fontSize: 12, color: 'var(--tx3)', padding: '7px 0', fontStyle: 'italic' }}>No medicines</div> : meds.map((m, i) => <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid rgba(255,255,255,0.04)' }}><div><div style={{ fontSize: 12, fontWeight: 500 }}>{m}</div><div style={{ fontSize: 9.5, color: 'var(--tx3)', fontFamily: "'JetBrains Mono',monospace", marginTop: 1 }}>As prescribed</div></div><div onClick={() => toggleTl(s.k, i, meds.length)} style={{ width: 21, height: 21, borderRadius: 6, border: `1.5px solid ${tlck[`${s.k}-${i}`] ? 'var(--tl)' : 'var(--b2)'}`, background: tlck[`${s.k}-${i}`] ? 'var(--tl)' : 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, color: tlck[`${s.k}-${i}`] ? 'var(--n)' : 'transparent', transition: 'all .2s', flexShrink: 0 }}>✓</div></div>)}{allDone && <div style={{ textAlign: 'center', marginTop: 10, fontSize: 12.5, color: 'var(--tl)', fontWeight: 600 }}>🎉 All done!</div>}</div>; })}
            </div>
          </Card>
          <Card><HDR icon="🔔" title="Family Reminder" chip="Live" cc="gold" /><p style={{ fontSize: 13, color: 'var(--tx2)', lineHeight: 1.7, marginBottom: 12 }}>When a dose is overdue, family gets an alert in {lang}.</p><div style={{ background: 'var(--go2)', border: '1px solid var(--b3)', borderRadius: 10, padding: '11px 15px', marginBottom: 14, fontSize: 13, color: 'var(--tx2)', fontStyle: 'italic' }}>"{lang === 'Hindi' ? `Beta, ${r.patientName || 'Patient'} ne abhi tak dawa nahi li 🙏` : lang === 'Gujarati' ? `Beta, ${r.patientName || 'Patient'} e davai nathi lidhi 🙏` : `Beta, ${r.patientName || 'Patient'} has not taken medicine yet 🙏`}"</div><div style={{ display: 'flex', gap: 8 }}><Btn primary sm onClick={() => { if (!('Notification' in window)) { showToast('⚠️ Not supported'); return; } Notification.requestPermission().then(p => showToast(p === 'granted' ? '✅ Enabled!' : '❌ Denied')); }}>🔔 Enable</Btn><Btn sm onClick={() => { const m = lang === 'Hindi' ? `Beta, ${r.patientName || 'Patient'} ne dawa nahi li` : `Beta, ${r.patientName || 'Patient'} has not taken medicine`; if (typeof Notification !== 'undefined' && Notification.permission === 'granted') new Notification('💊 DawaClear', { body: m }); showToast('📳 ' + m); }}>📳 Test</Btn></div></Card>
        </>;
      }

      case 'chemist': return <Card><HDR icon="💊" title="Chemist Dispense Sheet" chip={`${(r.medications || []).length} items`} /><p style={{ fontSize: 12.5, color: 'var(--tx2)', marginBottom: 12 }}>Hand your phone directly to the pharmacist.</p>{(r.medications || []).length === 0 ? <div style={{ fontSize: 13, color: 'var(--tx3)' }}>No medications found.</div> : <table style={{ width: '100%', borderCollapse: 'collapse' }}><thead><tr>{['Medicine', 'Dose', 'Qty', 'Timing', 'Days'].map(h => <th key={h} style={{ fontSize: 9.5, color: 'var(--tx3)', fontWeight: 600, textAlign: 'left', padding: '8px 9px', borderBottom: '1px solid var(--b2)', letterSpacing: '0.08em', textTransform: 'uppercase' }}>{h}</th>)}</tr></thead><tbody>{(r.medications || []).map(m => <tr key={m.name}><td style={{ padding: '10px 9px', fontSize: 13, color: 'var(--tx)', fontWeight: 600, borderBottom: '1px solid rgba(255,255,255,0.03)' }}>{m.name}</td><td style={{ padding: '10px 9px', fontSize: 12.5, color: 'var(--tx2)', borderBottom: '1px solid rgba(255,255,255,0.03)' }}>{m.dose}</td><td style={{ padding: '10px 9px', fontSize: 13.5, color: 'var(--tl)', fontWeight: 700, borderBottom: '1px solid rgba(255,255,255,0.03)' }}>{m.quantity || m.days}</td><td style={{ padding: '10px 9px', borderBottom: '1px solid rgba(255,255,255,0.03)' }}><span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 10, background: 'var(--tl3)', color: 'var(--tl)', padding: '2px 7px', borderRadius: 5 }}>{m.timing}</span></td><td style={{ padding: '10px 9px', fontSize: 12.5, color: 'var(--tx2)', borderBottom: '1px solid rgba(255,255,255,0.03)' }}>{m.days}</td></tr>)}</tbody></table>}<div style={{ marginTop: 14, display: 'flex', gap: 8 }}><Btn primary sm onClick={() => pChemist(r)}>🖨️ Print</Btn><Btn sm onClick={() => copy((r.medications || []).map(m => `${m.name} | ${m.dose} | ${m.quantity || m.days} | ${m.timing} | ${m.days}`).join('\n'))}>📋 Copy</Btn></div></Card>;

      case 'sideeffects': return <Card><HDR icon="⚠️" title="Side Effects — Color Coded" chip="Important" cc="red" />{[['🔴 Call Doctor Immediately', r.sideEffectsRed || [], 'var(--rd)'], ['🟡 Monitor Carefully', r.sideEffectsAmber || [], 'var(--am)'], ['🟢 Usually Mild', r.sideEffectsGreen || [], 'var(--gr)']].map(([label, items, col]) => <div key={label} style={{ marginBottom: 14 }}><div style={{ fontSize: 10.5, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.1em', color: col, marginBottom: 7 }}>{label}</div>{items.length === 0 ? <div style={{ fontSize: 12.5, color: 'var(--tx3)', padding: '5px 0' }}>None listed</div> : items.map(fx => <div key={fx} style={{ display: 'flex', alignItems: 'flex-start', gap: 8, padding: '6px 0', borderBottom: '1px solid rgba(255,255,255,0.04)' }}><div style={{ width: 6, height: 6, borderRadius: '50%', background: col, flexShrink: 0, marginTop: 5 }} /><div style={{ fontSize: 12.5, color: 'var(--tx2)', lineHeight: 1.5 }}>{fx}</div></div>)}</div>)}</Card>;

      case 'checklist': return <Card><HDR icon="✅" title="Follow-up Checklist" chip={`${Object.values(ck).filter(Boolean).length}/${(r.checklist || []).length} done`} />{(r.checklist || []).length === 0 ? <div style={{ fontSize: 13, color: 'var(--tx3)' }}>No checklist items.</div> : (r.checklist || []).map((t, i) => <div key={i} onClick={() => toggleCk(i)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 0', cursor: 'pointer', borderBottom: '1px solid rgba(255,255,255,0.04)' }}><div style={{ width: 18, height: 18, borderRadius: 5, border: `1.5px solid ${ck[i] ? 'var(--tl)' : 'var(--b2)'}`, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, background: ck[i] ? 'var(--tl)' : 'transparent', color: ck[i] ? 'var(--n)' : 'transparent', transition: 'all .2s' }}>✓</div><div style={{ fontSize: 12.5, color: ck[i] ? 'var(--tx3)' : 'var(--tx2)', textDecoration: ck[i] ? 'line-through' : 'none', flex: 1, transition: 'all .2s' }}>{t}</div></div>)}<div style={{ marginTop: 11, fontSize: 11.5, color: 'var(--tx3)' }}>{Object.values(ck).filter(Boolean).length} / {(r.checklist || []).length} completed</div></Card>;

      case 'family': return <><Card gold><HDR icon="💬" title="Share with Family" chip="One-liner" cc="gold" /><div style={{ fontSize: 13.5, color: 'var(--tx2)', lineHeight: 1.8, fontStyle: 'italic', marginBottom: 14 }}><span style={{ color: 'var(--go)', fontStyle: 'normal', fontWeight: 600 }}>"</span>{r.familyOneliner || 'No summary.'}<span style={{ color: 'var(--go)', fontStyle: 'normal', fontWeight: 600 }}>"</span></div><div style={{ display: 'flex', gap: 7, flexWrap: 'wrap' }}>{[['📋 Copy', () => copy(r.familyOneliner || '')], ['💬 WhatsApp', () => window.open(`https://wa.me/?text=${encodeURIComponent(r.familyOneliner || '')}`, '_blank')], ['🔊 Read Aloud', () => speak(r.familyOneliner || '')]].map(([l, fn]) => <button key={l} onClick={fn} style={{ display: 'flex', alignItems: 'center', gap: 4, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.09)', color: 'var(--tx2)', padding: '6px 13px', borderRadius: 100, fontSize: 11.5, cursor: 'pointer', fontFamily: "'Sora',sans-serif" }}>{l}</button>)}</div></Card><Card><HDR icon="🔔" title="Family Connect" chip="Beta" cc="gold" /><p style={{ fontSize: 13, color: 'var(--tx2)', lineHeight: 1.75, marginBottom: 13 }}>Enable browser notifications so family gets alerted for missed doses in <strong style={{ color: 'var(--tl)' }}>{lang}</strong>.</p><div style={{ background: 'var(--go2)', border: '1px solid var(--b3)', borderRadius: 10, padding: '11px 15px', marginBottom: 14, fontSize: 13, color: 'var(--tx2)', fontStyle: 'italic' }}>"{lang === 'Hindi' ? `Beta, ${r.patientName || 'Patient'} ne abhi tak dawa nahi li 🙏` : lang === 'Gujarati' ? `Beta, ${r.patientName || 'Patient'} e davai nathi lidhi 🙏` : `Beta, ${r.patientName || 'Patient'} has not taken medicine yet 🙏`}"</div><div style={{ display: 'flex', gap: 8 }}><Btn primary sm onClick={() => Notification.requestPermission().then(p => showToast(p === 'granted' ? '✅ Enabled!' : '❌ Denied'))}>🔔 Enable</Btn><Btn sm onClick={() => { const m = `Beta, ${r.patientName || 'Patient'} has not taken medicine`; if (Notification.permission === 'granted') new Notification('💊', { body: m }); showToast('📳 ' + m); }}>📳 Test</Btn></div></Card></>;

      case 'questions': return <Card><HDR icon="❓" title="Questions to Ask Your Doctor" chip={`${(r.doctorQuestions || []).length} questions`} /><p style={{ fontSize: 12.5, color: 'var(--tx2)', marginBottom: 13 }}>Generated from your specific diagnosis and medications:</p>{(r.doctorQuestions || []).length === 0 ? <div style={{ fontSize: 13, color: 'var(--tx3)' }}>No questions generated.</div> : (r.doctorQuestions || []).map((q, i) => <div key={i} style={{ padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,0.04)', fontSize: 12.5, color: 'var(--tx2)', display: 'flex', alignItems: 'flex-start', gap: 8, lineHeight: 1.55 }}><span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 10, color: 'var(--tl)', flexShrink: 0, marginTop: 2 }}>{String(i + 1).padStart(2, '0')}</span>{q}</div>)}<div style={{ marginTop: 13, display: 'flex', gap: 8 }}><Btn primary sm onClick={() => copy((r.doctorQuestions || []).map((q, i) => `${i + 1}. ${q}`).join('\n'))}>📋 Copy All</Btn><Btn sm onClick={() => speak('Questions for your doctor: ' + (r.doctorQuestions || []).join('. '))}>🔊 Read</Btn></div></Card>;
    ...
    <div style={{ position: 'relative', zIndex: 1, minHeight: '100vh', paddingTop: 64 }}>
      ...
      <div style={{ background: 'rgba(5,13,26,0.9)', backdropFilter: 'blur(20px)', borderBottom: '1px solid var(--b2)', padding: '12px 1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8, position: 'sticky', top: 64, zIndex: 100 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div><div style={{ fontSize: 14, fontWeight: 600 }}>{r.patientName || 'Patient'}</div><div style={{ fontSize: 11, color: 'var(--tx3)', fontFamily: "'JetBrains Mono',monospace", marginTop: 1 }}>{[r.patientAge ? r.patientAge + 'y' : '', r.diagnosisTitle, r.date].filter(Boolean).join(' · ')}</div></div>
        </div>
...
export default function Home() {
  ...
  const handleStart = async ({ file, fdu, name, age, lang: l, mode, role }) => {
    setLang(l); setScreen('processing');
    ...
    try {
      ...
      const res = await fetch('/api/analyse', {
        ...
        body: JSON.stringify({ fileDataUrl: fdu, fileType: file.type, fileName: file.name, age, name, lang: l, mode }),
      });
  ...
  const loadDemo = () => { setResult({ ...DEMO }); setLang('English'); setScreen('dashboard'); showToast('👁 Demo loaded!'); };
  const cycleLang = () => { const ll = ['English', 'Hindi', 'Gujarati']; const n = ll[(ll.indexOf(lang) + 1) % 3]; setLang(n); showToast('🌐 Language → ' + n); };
  ...
  return (
    <>
      ...
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 200, height: 64, padding: '0 1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(5,13,26,0.9)', backdropFilter: 'blur(28px)', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          {screen === 'dashboard' && <span style={{ fontSize: 11, color: 'var(--tx3)', fontFamily: "'JetBrains Mono',monospace", padding: '4px 10px', background: 'var(--gl)', border: '1px solid var(--b2)', borderRadius: 100 }}>📄 {result?.patientName || 'Patient'}</span>}
          <button onClick={() => setScreen('landing')} style={{ background: 'linear-gradient(135deg,var(--tl),var(--tl2))', color: 'var(--n)', fontWeight: 700, padding: '7px 18px', borderRadius: 100, border: 'none', fontSize: 12, cursor: 'pointer', fontFamily: "'Sora',sans-serif" }}>{screen === 'dashboard' ? '+ New Document' : 'Try Free →'}</button>
        </div>
      </nav>
